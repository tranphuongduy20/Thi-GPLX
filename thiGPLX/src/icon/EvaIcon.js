import React from "react";
import { Icon } from "@ui-kitten/components";

export const HomeIcon = (props) => <Icon name="home-outline" {...props} />;

export const TestIcon = (props) => <Icon name="file-text-outline" {...props} />;

export const CarIcon = (props) => <Icon name="car" {...props} />;

export const SignIcon = (props) => <Icon name="alert-circle" {...props} />;

export const AwardIcon = (props) => <Icon name="award" {...props} />;

export const SearchIcon = (props) => <Icon name="search-outline" {...props} />;

export const BackIcon = (props) => <Icon {...props} name="arrow-back" />;

export const BookIcon = (props) => <Icon name="book" {...props} />;

export const ListIcon = (props) => <Icon name="list" {...props} />;

export const ArrowBackIcon = (props) => (
  <Icon name="arrow-ios-back" {...props} />
);

export const ArrowForwardIcon = (props) => (
  <Icon name="arrow-ios-forward" {...props} />
);
